...
class Template:
    ...
    def apply (self, element):
        return self.apply_templates(element.xpath("./*"))
...
class SplittableInlineContainerTemplate (Template):
    ...
    @returns_blocks
    def apply (self, element):
        yield from self.segmented(element.xpath("./node()"))
...
class TextTemplate (Template):
    @classmethod
    def admits (cls, element):
        return is_text_element(element)
...
class ListTemplate (Template):
    @returns_block
    def apply (self, element):
        subnodes = element.xpath("./node()")

        # ⚠ <ul><ul>...</ul></ul> and other platypi spotted in the wild!
        while (len(subnodes) == 1 and tag(subnodes[0]) == self.tag):
            subnodes = subnodes[0].xpath("./node()")

        content = []
        for subnode in subnodes:
            subsubnodes = (subnode.xpath("./node()") if tag(subnode) == "li"
                           else [subnode])
            content.append(dict(
                type="list_item",
...
class Template__a (Template):
    ...
    def apply (self, element):
        ...
        transformed = self.apply_templates(element.xpath("./node()"))
...
def is_text_element (element):
    return hasattr(element, "splitlines")
...
class _LxmlNamespacedMembrane:
    ...
    def xpath (self, _path, *args, **kwargs):
        ...
        return [
            r if is_text_element(r) else _LxmlNamespacedMembrane(r)
            for r in ret
        ]
